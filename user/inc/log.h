...
#define LOG_ERROR(fmt, ...) eat_trace("%ld[%d][ERR][%s:%d %s]"fmt, rtc_getTimestamp(), eat_get_task_id(), __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__) ;\
                            log_file("%ld[%s:%d %s]"fmt, rtc_getTimestamp(), __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)
#define LOG_HEX(data, len)  log_hex(data, len);
#else
#define LOG_DEBUG(fmt, ...)
#define LOG_INFO(fmt, ...)
#define LOG_ERROR(fmt, ...)  log_file("%ld[%s:%d %s]"fmt, rtc_getTimestamp(), __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)
